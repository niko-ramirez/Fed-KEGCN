...
flags = tf.app.flags
FLAGS = flags.FLAGS
flags.DEFINE_string('dataset', 'aifb', 'Dataset: fb15k, am, wordnet, aifb.')
...
# Load data
adj, num_ent, train, test, valid, y, KG = load_data_class(FLAGS)

print ("train: ", len(train))
print("y: ", y.shape)
print("adj: ")
print(adj[0].get_shape())


# def split_graph(adj, length):
#     client_edge_num_1 = int(length*2/4)
#     client_edge_num_2 = int(length*1/4)
#     client_edge_num_other = int(length*1/32)
#     edge_num_list = [client_edge_num_1, client_edge_num_2] + [client_edge_num_other]*8
#     if client_edge_num_1 + client_edge_num_2 + 8*client_edge_num_other != length:
#         edge_num_list[0] = client_edge_num_1 + length - (client_edge_num_1 + client_edge_num_2+ 8*client_edge_num_other)
#     client_adj = []
#     row_start = 0
#     adj_csr = adj.tocsr()
#     for row_num in edge_num_list:
#         copy_adj = adj_csr.copy()
#         copy_adj[0:row_start] = 0
#         copy_adj[row_start+row_num:] = 0
#         client_adj.append(copy_adj)
#         row_start += row_num
#     return client_adj

# length = adj[0].get_shape()[0]
# client_adj = split_graph(adj[0], length)
# adj = client_adj[0]
# print(adj)
# print("new_adj:")
# print(adj[0].get_shape())
...
# 30 for fb15k
# num_bases = 20 for rgcn with aifb
model = model_func(placeholders, input_dim, hidden_dim, output_dim, dataset=FLAGS.dataset,
                    ...
                    logging=True, rel_update=FLAGS.rel_update, basis_decomp=True, num_bases = 20, task=task)
...
for epoch in range(FLAGS.epochs):
    ...
    if epoch % 10 == 0:
        ...
        # temp = [(i,val) for i, val in model.vars.items()]
        # logging.info(f"Model_vars: {temp}")
