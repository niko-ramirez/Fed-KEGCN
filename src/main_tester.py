...
def classification(args):
    ...
    flags.DEFINE_string('dataset', 'wordnet', 'Dataset: am, wordnet, aifb.')
    ...
    #flags.DEFINE_string('optim', 'GD', 'Optimizer: GD, Adam')
    ...
    flags.DEFINE_integer('dim', 16, 'hidden Dimension')
    ...
    for rate, basis, kge in my_combos:
        ...
        flags.DEFINE_float('basis', basis, 'Initial learning rate.', allow_override=True)
        flags.DEFINE_string('mode', kge, 'KE method for GCN: TransE, TransH, TransD, DistMult, RotatE, QuatE', allow_override=True)
        ...
        # real_adj = adj[-1]
        client_data = {}
        #data = {"train": og_train, "test": test, "valid": valid, "y": y}
        ...
        placeholders = {
            ...
            'basis_dict': tf.placeholder(tf.float32, None)
        }
        ...
        top_level_session = tf.Session(config= config)
        ...
        ser_model =  model_func(placeholders, input_dim, hidden_dim, output_dim, dataset=FLAGS.dataset,
                            train_labels=og_train, mode=FLAGS.mode, embed=FLAGS.embed, alpha=FLAGS.alpha,
                            beta=FLAGS.beta, layer_num=FLAGS.layer, basis_decomp = True, num_bases = basis, sparse_inputs=False, featureless=True,
                            logging=True, client_id = -1, rel_update=FLAGS.rel_update, task=task, session = top_level_session)
        ...
        total_clients = 3
        init_basis_dict = create_basis_dict(total_clients, ser_model.vars)
        top_level_session.run(tf.global_variables_initializer())
        ...
        for i in range(total_clients):
            model =  model_func(placeholders, input_dim, hidden_dim, output_dim, dataset=FLAGS.dataset,
                            ...
                            client_id=i, basis_dict=init_basis_dict, rel_update=FLAGS.rel_update, task=task, session = top_level_session)

            client = Client(i, client_data[i], model, FLAGS, FLAGS.epochs, FLAGS.learning_rate, 0, '../log/state/' + "Single" + "/", logging, get_eval)
            clients.append(client)
            print("client: ", i)
            print(client_nodes_num[i])
            print(len(client_data[i]["train"]))
        
        return server, clients, FLAGS


def create_basis_dict(client_number, server_vars):
    basis_dict = {}
    for i in range(client_number):
        client_dict = {}
        for model_var_name, var in server_vars.items():
            if model_var_name.endswith('ent_weights_0:0'):
                ...
                if key == 0:
                    key = 2
                client_dict[key-1] = tf.Variable(var, name="client_" + str(i)+"_var_"+str(key)+"_")
        basis_dict[i] = client_dict
    return basis_dict
    ...
    for t in range(20):
        ...
        for i, client in enumerate(clients):
            if t != 0:
                ...
                client.getGrad(*server.sendGrads())
            client.train()
            server.getParame(*client.uploadParame())
            updated_basis = client.getBasis()
            client_basis_dict[i] = updated_basis
            for i, client_2 in enumerate(clients):
                client_2.setAllBasis(client_basis_dict)
